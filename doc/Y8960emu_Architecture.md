...
キーオン/オフ状態を再び変える書き込みが来たとき、その書き込みを保留し、前の状態の
まま約2ms分を生成してから適用する。保留中の生成は `generate()` の呼び出しをまたいで
数え、後続の書き込みも順序を保って待たせる。このため衝突が多いときや呼び出しが
細かいときは、書き込みの適用が衝突1回につき最大約2ms遅れて累積する（未観測の
キー状態を捨てないことを、タイミングの正確さより優先している）。ymfm はキー状態をサンプル生成時にしか
